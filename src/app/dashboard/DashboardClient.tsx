...
import { HardHat, FileText, Calendar, CheckCircle, Clock, Loader2 } from 'lucide-react';
...
export default function DashboardClient({ userName }: { userName: string }) {
    ...
    const doFetchDashboardData = async () => {
        ...
        try {
            ...
            if (activeProjects > 0) {
                activityList.push({
                    id: 'proj-1', type: 'project', title: 'Project Update',
                    description: `You have ${activeProjects} active roofing project(s).`,
                    time: 'Recent', status: 'active'
                });
            }
            if (pendingQuotes > 0) {
                activityList.push({
                    id: 'quote-1', type: 'quote', title: 'Pending Quote',
                    description: `You have ${pendingQuotes} estimate(s) awaiting review.`,
                    time: 'Action Required', status: 'pending'
                });
            }
            if (upcomingInspection) {
                activityList.push({
                    id: 'insp-1', type: 'inspection', title: 'Inspection Scheduled',
                    description: `Your roof assessment is set for ${nextInspection}.`,
                    time: 'Upcoming', status: 'active'
                });
            }
        ...
        <div>
            ...
            <div className="grid-4">
                <div className="card stat-card">
                    <div className="stat-icon stat-icon-orange"><HardHat size={20} /></div>
                    <div>
                ...
                <div className="card stat-card">
                    <div className="stat-icon stat-icon-purple"><Calendar size={20} /></div>
                    <div>
            ...
            <div className="grid-2" style={{ marginTop: 24 }}>
                <div className="card">
                    ...
                    <div className="timeline">
                        {activities.length === 0 ? (
                            <p style={{ color: 'var(--slate-500)', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>
                ...
                <div className="card">
                    ...
                    <div style={{ display: 'grid', gap: 12 }}>
                        <button className="btn btn-secondary" style={{ justifyContent: 'flex-start', padding: 16 }}
                            onClick={() => window.location.href = '/dashboard/book-inspection'}>
